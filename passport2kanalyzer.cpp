...
Passport2KAnalyzer::Passport2KAnalyzer(QObject *parent, const QString &_docxPath) : QObject(parent)
{
    ...
    //    qDebug() << "passport thread: " << this->thread();
    //    if (!this->Documents_)
...
bool Passport2KAnalyzer::Run(const QString &_docxPath)
{
    this->WordApp_ = new QAxObject("Word.Application");
    if (!this->WordApp_)
    {
        emit this->signalInfoToUIfailTextBox("Не создался Word.Application. Зови на помощь//PA");
        return false;
    }
    this->Documents_ = WordApp_->querySubObject("Documents()");
    if (!this->Documents_)
    {
        emit this->signalInfoToUIfailTextBox("Не создался Documents(). Зови на помощь");
        return false;
    }
    this->docxPath_ = _docxPath;
    this->Documents_->querySubObject("Open(const QString)", this->docxPath_);
...
void Passport2KAnalyzer::SetDocxPath(const QString &_docxPath)
{
...
void Passport2KAnalyzer::Initialize()
{
...
void Passport2KAnalyzer::EjectOsnastka()
{
    int intCols = this->Table_->querySubObject("Range")->querySubObject("Columns")->property("Count").toInt();
    int intRows = this->Table_->querySubObject("Range")->querySubObject("Rows")->property("Count").toInt();
...
void Passport2KAnalyzer::EjectDeveloperFromTable()
{
    int intRows = this->Table_->querySubObject("Range")->querySubObject("Rows")->property("Count").toInt();
    int intCols = this->Table_->querySubObject("Range")->querySubObject("Columns")->property("Count").toInt();
...
void Passport2KAnalyzer::EjectDeveloperFromPlainText()
{
...
void Passport2KAnalyzer::EjectTesters()
{
    int intCols = this->Table_->querySubObject("Range")->querySubObject("Columns")->property("Count").toInt();
    int intRows = this->Table_->querySubObject("Range")->querySubObject("Rows")->property("Count").toInt();
...
void Passport2KAnalyzer::EjectSeries()
{
...
void Passport2KAnalyzer::EjectTYandCorrection()
{
...
const QStringList &Passport2KAnalyzer::testers() const
{
    return this->slTesters_;
}

const QString &Passport2KAnalyzer::TY() const
{
    return this->TY_;
}

const QString &Passport2KAnalyzer::TYcorrection() const
{
    return this->TYcorrection_;
}

const  QString &Passport2KAnalyzer::KY() const
{
    return this->KY_;
}

const QString &Passport2KAnalyzer::developer() const
{
    return this->developer_;
}

const QVector<RowInExcelTable> Passport2KAnalyzer::rowsInExcelTable() const
{
...
int Passport2KAnalyzer::DocumentsCount() const
{
    return this->Documents_->property("Count").toInt();
}

void Passport2KAnalyzer::PrepareNameInsertSpace(QString &_name)
{
